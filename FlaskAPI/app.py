...
def load_models():
    file_name = 'models/model_file_updated.p' #try lin_model
    
    with open(file_name, 'rb') as pickled:
        data = pickle.load(pickled)
        model = data['model'] #lin_model or model, depending on model to use
    return model
...
def predict(sample):
    ...
    prediction = model.predict(sample) #do we need [0]?
